/*
 *  fft_datacollection.ino - FFT data collection sample for Sound Analysis
...
void setup() {
  ...
  int err = theAudio->initRecorder(AS_CODECTYPE_PCM 
    ,"/mnt/sd0/BIN", AS_SAMPLINGRATE_192000, AS_CHANNEL_MONO);                             
...
void loop() {
  ...
  // 0.5秒経過したら記録する
  if (capture_interval > 500) {
    theAudio->stopRecorder(); // 録音停止
    // saveData関数：SDカードにデータを記録  
    //   データへのポインタ（pDst）
    //   記録データのサイズ（FFT_LEN/8）
    //   データ保存数（500）
    //   24kHzの1/8 なので、3kHz成分まで
    saveData(pDst, FFT_LEN/8, 500); 
...
void saveData(float* pDst, int dsize, int quantity) {
  ...
  // データの書き込み
  for (int i = dc_offset; i < dsize + dc_offset; ++i) {
    myFile.println(String(pDst[i],4));
  }
