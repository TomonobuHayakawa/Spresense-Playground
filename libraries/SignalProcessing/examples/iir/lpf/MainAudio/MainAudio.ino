...
void loop()
{
  ...
  switch (state) {
    ...
    case StatePrepare:
    {
      ...
      if (err != 0) {printf("Error! %x ,%x , %d\n",err,capture.buffer,capture.sample);}
 
      if (read_size > 0)
      {
        capture.buffer  = s_buffer;
        capture.sample = read_size/ sizeof(uint16_t) / mic_channel_num;
        ...
        if (ret >= 0) {
        if (result->sample != 768) {puts("kokoko"); break;}
