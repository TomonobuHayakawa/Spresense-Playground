...
void setup() {
  ...
  uint32_t wait_time = Zeta.connect();
  sleep(wait_time);
...
void loop()
{
  ...
  float temp = sensor0.readTempC();
  Serial.println(temp);
  sensor0.sleep(); // Return sensor to sleep

  uint32_t palam = temp * 10;
  
  //温度データをpacketへの格納　JAZE温湿度センサJZTH91A1に準拠
  packet[0] = 01;//ヘッダ
  packet[1] = (palam & 0x0000FF00) >> 8;//温度
  packet[2] = (palam & 0x000000FF);//温度
  packet[3] = 00;//（湿度実装なし）

  Zeta.send(packet, sizeof(packet));
  sleep(20);
